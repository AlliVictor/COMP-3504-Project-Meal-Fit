...
app.post('/api/meals/plan', async (req, res) => {
        ...
        try {
            ...
            if (restrictionsArray.length > 0) {
                // Build query with LIKE operators for partial matches
                const conditions = restrictionsArray
                    .map(() => `dietrestrictions.restriction_name LIKE ?`)
                    .join(' OR ');
                ...
                // Add wildcards to params for partial matching
                params = restrictionsArray.map((r) => `%${r}%`);
            } else {
